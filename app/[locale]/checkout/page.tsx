...
import { Phone, CheckCircle2, AlertCircle, Loader2, ArrowRight, ShieldCheck, CreditCard, Timer, Zap } from 'lucide-react';
...
function CheckoutContent() {
  ...
  const handlePaystackPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We need to wait for state update but initializePayment uses the config passed at render time
      // So we have to ensure transactionId is set before calling it, or pass config directly if possible.
      // Since usePaystackPayment is a hook, we trigger it only when transactionId is ready.
    } catch (err: any) {
