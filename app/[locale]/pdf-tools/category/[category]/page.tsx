...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; category: string }> }) {
    ...
    return {
        title: `${formattedCategory} Tools - Akubrecah Dev Cave PDF`,
...
export default async function CategoryPage({ params }: { params: Promise<{ locale: string; category: string }> }) {
    ...
    const { PDF_TOOLS: tools } = await import('@/config/pdf-tools');
    const { getToolContent } = await import('@/config/pdf-tool-content');
