...
import { Search, Bell, User, RefreshCw, Home, MessageSquare, Sparkles } from 'lucide-react';
    ...
    const fetchPendingCount = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const count = messages.filter((m) => m.status === 'pending').length;
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 glass-panel border border-white/10 rounded-[20px] shadow-2xl bg-white/5 backdrop-blur-md">
            ...
            <button 
              ...
              className="p-2.5 rounded-xl hover:bg-[var(--color-brand-red)]/20 transition-all group relative border border-transparent hover:border-[var(--color-brand-red)]/30"
              title="Specialist Agents Dashboard"
            >
              <Sparkles className="w-4 h-4 text-[var(--color-brand-red)] group-hover:scale-110 transition-transform" />
              <div className="absolute -top-1 -right-1 w-2 h-2 bg-emerald-500 rounded-full animate-pulse border border-black" />
            </button>
      ...
      <Modal
        ...
        title="SPECIALIST AGENT HUB"
        size="xl"
        className="!bg-[#0A0A0A] border border-[var(--color-brand-red)]/20"
      >
        <AgentDashboard />
      </Modal>
    </>
